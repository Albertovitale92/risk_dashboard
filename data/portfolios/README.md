...
This folder holds mock portfolio inputs for a future portfolio-risk layer and
the mapping from each position to the shared `Risk_Factors` time series.
...
- `risk_factor_mapping.csv` maps each position to one or more risk factors from
  `Risk_Factors\data\raw\risk_factors_levels.parquet`.
- `risk_factors.csv` lists the currently available shared risk factors and their intended use.
...
Risk-factor names should match the column names in the shared Parquet store whenever possible. The available historical risk factors include:
